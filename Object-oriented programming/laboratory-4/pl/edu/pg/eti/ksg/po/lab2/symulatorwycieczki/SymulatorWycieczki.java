...
public class SymulatorWycieczki {
    ...
    private Set<SluchaczPostepow> sluchacze;
    public SymulatorWycieczki(Grupa grupa, Wycieczka wycieczka) {
        this.sluchacze = new HashSet<>();
        this.grupa = grupa;
    ...
    public void symuluj() {
        ...
        for(pozycjaGrupy = 0; pozycjaGrupy < wycieczka.getLiczbaElementowWycieczki(); pozycjaGrupy++) {
            ...
            else if(elementWycieczki instanceof Atrakcja)
            {
                ...
                System.out.println("Grupa zwiedza: " + atrakcja.getNazwa());

                double predkoscGrupy = grupa.getPredkosc();

                for(Uczestnik u : grupa.getUczestnicy()) {
                    u.reagujNaAtrakcje(atrakcja);
                }

                System.out.println("Atrakcja zajęła grupie " + atrakcja.getCzasZwiedzania() + " h");
                czasWycieczki += atrakcja.getCzasZwiedzania();
            }

            for(SluchaczPostepow sp: this.sluchacze)
            {
                sp.aktualizujPostep(elementWycieczki, pozycjaGrupy, wycieczka.getLiczbaElementowWycieczki());
            }
            System.out.println();
